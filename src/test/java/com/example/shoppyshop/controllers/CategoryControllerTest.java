...
class CategoryControllerTest {
  ...
  void findCategoriesTest(@Autowired MockMvc mvc) throws Exception {
    ...
    mvc.perform(get("/admin/api/categories"))
        ...
        .andExpect(
            content().string("[{\"name\":\"test category 1\"},{\"name\":\"test category 2\"}]"));
  }
  ...
  void categoriesTest_Unauthorized(@Autowired MockMvc mvc) throws Exception {
    ...
    mvc.perform(get("/admin/api/categories")).andExpect(status().isForbidden());
    mvc.perform(get("/admin/api/categories/" + c1.getId())).andExpect(status().isForbidden());
    mvc.perform(
  ...
  void findByIdTest_NotFound(@Autowired MockMvc mvc) throws Exception {
    mvc.perform(get("/admin/api/categories/0")).andExpect(status().isNotFound());
  }
  ...
  void updateTest(@Autowired MockMvc mvc) throws Exception {
    ...
    mvc.perform(
            put("/admin/api/categories/" + c1.getId())
